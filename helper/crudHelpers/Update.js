  ...
  try {
          ...
          return new Promise((resolve, reject) => {
            ...
            if (FilesArray.length > 0) {
              ...
              let movePath = path.join(
                __dirname,
                `../../assets/${filesPath}${VideoOrImage}/${value[1].md5 +
                randomNumber +
                "." +
              ...
              let urlPath =
                process.env.LIVEURL +
                `/${filesPath}/${VideoOrImage}/${value[1].md5 +
                randomNumber +
                "." +
                value[1].mimetype.split("/")[1]
                }`;
    ...
    promimse_all.then(async (v) => {
      ...
      try {
        ...
        return res.status(200).json({ message: "error", data: [], detail: error.code === 11000 ? ("Country Code Already Taken!") : error.message });
      }
    ...
    return res.status(200).json({ message: "error", data: [], detail: err.message });
  }
};
